...
| `js/main.js` (section auto-save), `js/templates.js` | `autosave` (+ `comments` si `loadForTemplate`/`onSave` change) |
| `js/comments.js`, `js/editor-nodes.js:createCommentMark` | `comments` |
| `js/page-layout.js`, `js/settings.js` (marges) | `autosave` (les marges partent dans le même `Templates.save()`) + les groupes de rendu du domaine touché |
...
node dev-tests/run-headless.mjs autosave comments   # seulement ces groupes
```
...
const files = [
  ...
  'scenarios-autosave', 'scenarios-comments',
];

...
| v0.9 | 2026-09-07 | `0271491` | **stable** ✅ | Suite de correctifs sur la manipulation et l'export des images en calque, dans un document créé/testé à neuf. Glisser-déposer : plus de duplication au changement de calque (`draggable` toujours `false` sur l'image), une nouvelle image démarre directement en calque "devant" (au lieu d'un état "normal" peu utilisable), glisser-déposer fonctionnel derrière le texte via le marqueur d'ancrage dédié, alignement gauche/centre/droite qui reste utilisable pendant le déplacement (aligne/recentre l'image plutôt que de se désactiver). Export PDF : paragraphe ne contenant qu'une image (aucun texte) qui n'obtenait aucune position exploitable, ancrage qui privilégiait à tort le premier paragraphe venu en cas d'égalité de distance ou de chevauchement avec plusieurs lignes courtes voisines (préfère désormais le paragraphe réellement recouvert, puis le plus substantiel en cas d'égalité), marge de paragraphe appliquée à tort à une ligne vide (aucune marge visible entre lignes vides dans l'éditeur), base horizontale d'ancrage non tributaire d'un détail d'implémentation pdfmake sur les paragraphes centrés/alignés, et un `data-pm-anchor-id` qui pouvait se dupliquer sur plusieurs paragraphes après une scission de bloc (retour à la ligne près d'une image déjà ancrée) faisant résoudre l'ancre au hasard de l'ordre du DOM. Cache navigateur : toutes les ressources JS/CSS locales portent désormais un paramètre `?v=` à incrémenter à chaque changement (cf. `index.html`), pour éviter qu'une correction déjà poussée semble sans effet. **Connu, hors périmètre de cette version** : un document créé/édité AVANT ces correctifs peut contenir des attributs d'ancrage hérités (legacy) qui faussent encore l'export — signalé par l'utilisateur sur un document ancien, non reproduit sur un document neuf ; à investiguer séparément si confirmé. |

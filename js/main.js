...
(function () {
  ...
  async function init() { try { await GristAPI.init(); } catch (e) { setStatus('Erreur init API Grist.', true); } quill = Editor.init(); Variables.initFilenameInput(getPdfFilenameInput()); Variables.refreshLinkRulesPanel(); GristAPI.onRecord(async function (record, tableId) { latestRecord = record; latestRecordTableId = tableId || GristAPI.getCurrentTableId(); if (tableId) currentTableId = tableId; updateTableIndicator(latestRecordTableId); if (currentMode === 'read' && record) await renderReader(record, latestRecordTableId); }); try { if (grist.onOptions) grist.onOptions(() => {}); } catch (e) {} try { if (grist.onRecords) grist.onRecords(() => {}); } catch (e) {} await refreshTemplateList(); await onTemplateSelectChange(); templateSelect.addEventListener('change', onTemplateSelectChange); document.getElementById('btn-new').addEventListener('click', onNew); document.getElementById('btn-save').addEventListener('click', onSave); document.getElementById('btn-save-as').addEventListener('click', onSaveAs); document.getElementById('btn-delete').addEventListener('click', onDelete); btnEdit.addEventListener('click', () => switchMode('edit')); btnRead.addEventListener('click', () => switchMode('read')); document.getElementById('btn-export-pdf').addEventListener('click', onExportPdf); document.getElementById('btn-save-attachment').addEventListener('click', onSaveToAttachment); const toggleA4 = document.getElementById('toggle-a4-preview'); if (toggleA4) { const a4Label = toggleA4.closest('.a4-toggle'); const syncA4Checked = () => { editorContainer.classList.toggle('a4-preview', toggleA4.checked); if (a4Label) a4Label.classList.toggle('checked', toggleA4.checked); }; toggleA4.addEventListener('change', syncA4Checked); syncA4Checked(); } const btnTogglePanel = document.getElementById('btn-toggle-panel'); const toolbarPanel = document.getElementById('toolbar-panel'); if (btnTogglePanel && toolbarPanel) { btnTogglePanel.addEventListener('click', () => { const willOpen = toolbarPanel.hidden; toolbarPanel.hidden = !willOpen; btnTogglePanel.setAttribute('aria-expanded', String(willOpen)); if (willOpen && templateNameInput) templateNameInput.focus(); }); } updateTableIndicator(GristAPI.getCurrentTableId()); await switchMode('edit'); setStatus('Widget prêt.'); }
  init();
})();

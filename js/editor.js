...
const Editor = (function () {
  ...
  FontFamily.whitelist = ['arial', 'times-new-roman', 'courier-new', 'georgia', 'verdana'];
  ...
  PageBreakBlot.create = function (value) {
    const node = super.create(value);
    node.setAttribute('contenteditable', 'false');
    node.innerHTML = '<span>⏎ Saut de page (export PDF)</span>';
    return node;
  };
  Quill.register(PageBreakBlot);

  class EditableTableBlot extends Quill.import('blots/block/embed') {
    static create(value) {
    ...
    static value(node) { return { html: node.querySelector('table') ? node.querySelector('table').outerHTML : '' }; }
  }
  EditableTableBlot.blotName = 'editabletable';
  EditableTableBlot.tagName = 'div';
  Quill.register(EditableTableBlot);

  class TwoColumnsBlot extends Quill.import('blots/block/embed') {
    static create(value) {
      const node = super.create();
  ...
  function init() {
    ...
    const undoBtn = toolbar && toolbar.querySelector('.ql-undo');
    const redoBtn = toolbar && toolbar.querySelector('.ql-redo');
    const twoColsBtn = toolbar && toolbar.querySelector('.ql-insert-two-columns');
    const pageBreakBtn = toolbar && toolbar.querySelector('.ql-page-break');
    if (undoBtn) { undoBtn.innerHTML = '↶'; undoBtn.title = 'Annuler'; }
    if (redoBtn) { redoBtn.innerHTML = '↷'; redoBtn.title = 'Rétablir'; }
    if (twoColsBtn) {
    ...
    tableTools.addEventListener('click', function (event) {
      ...
      if (!table || !row) return;
      const makeCell = () => { const cell = document.createElement('td'); cell.innerHTML = '&nbsp;'; cell.contentEditable = 'true'; return cell; };
    ...
    quill.root.addEventListener('input', function () { quill.update(Quill.sources.USER); });
  }

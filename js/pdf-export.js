...
// Le mode texte natif reçoit le HTML déjà résolu par ReaderMode.preview et le convertit en structure pdfmake.
const PdfExport = (function () {
  ...
  function inlineRuns(node, parentStyle) { const style = inheritedStyle(node, parentStyle || { fontSize: DEFAULT_FONT_SIZE }); if (node.nodeType === Node.TEXT_NODE) return node.nodeValue ? [{ text: node.nodeValue, ...style }] : []; if (node.nodeType !== Node.ELEMENT_NODE) return []; if (node.classList.contains('page-break-marker')) return []; if (node.classList.contains('var-badge')) return [{ text: node.textContent || '', ...style }]; if (node.tagName === 'BR') return [{ text: '\n', ...style }]; let runs = []; node.childNodes.forEach(child => { runs = runs.concat(inlineRuns(child, style)); }); return runs; }
  ...
  function tableFrom(node, pageBreakBefore) {
    ...
    // Count the actual layout grid, including colspan, rather than the number
    // of DOM cells. This keeps pdfmake widths/body columns synchronized.

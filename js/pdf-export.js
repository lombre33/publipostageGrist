...
const PdfExport = (function () {
  ...
  function alignment(node) {
    let current = node;
    while (current) {
      const cls = current.classList || { contains: () => false };
  ...
  function tableFrom(node, pageBreakBefore) {
    ...
    const body = rawRows.map(row => {
      ...
      cellsOf(row).forEach(cell => {
        ...
        const cellRuns = inlineRuns(cell, { fontSize: DEFAULT_FONT_SIZE });
        const value = cellRuns.length ? cellRuns : [{ text: ' ', fontSize: DEFAULT_FONT_SIZE }];
        const cellDef = { stack: value, colSpan, rowSpan, margin: [4, 3, 4, 3] };
        output.push(cellDef);
    ...
    const table = { table: { headerRows: 0, widths: Array(columnCount).fill('*'), body }, layout: 'lightHorizontalLines', margin: [0, 6, 0, 6] };
    if (pageBreakBefore) table.pageBreak = 'before';
    return table;
  }
  function blockFrom(node, pageBreakBefore) { const tag = node.tagName.toUpperCase(); const runs = inlineRuns(node, { fontSize: HEADING_SIZES[tag] || DEFAULT_FONT_SIZE }); const block = { text: runs.length ? runs : ' ', margin: [0, 2, 0, 4] }; if (/^H[1-6]$/.test(tag)) block.fontSize = HEADING_SIZES[tag]; if (tag === 'LI') block.listType = 'none'; const a = alignment(node); if (a) block.alignment = a; if (pageBreakBefore) block.pageBreak = 'before'; return block; }
  function twoColumnsFrom(node, pageBreakBefore) {
    const cols = Array.from(node.querySelectorAll(':scope > .two-columns-column')).slice(0, 2).map(col => ({ stack: htmlToPdfContent(col.innerHTML), width: '*' }));
    const block = { columns: cols, columnGap: 16, margin: [0, 6, 0, 6] };
